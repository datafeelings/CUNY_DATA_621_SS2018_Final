...
summaries = function(data,var){
  ...
  c = scat621(data, var, log(price))                   #price vs variable
  d = boxcoxfit(var[!is.na(var)] + 0.001)              #boxcox
  return(list(a,b,c,d))
}
```

```{r load full data, echo=FALSE, message=FALSE, warning=FALSE}
# Load the full data
...
```{r host_response_rate dist, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r host_response_rate boxcox, echo=FALSE, message=FALSE, warning=FALSE}
boxcoxfit(inp$host_response_rate[!is.na(inp$host_response_rate)]+0.001)
```
  
#### Relationship to Price  
  
```{r host_response_rate price, echo=FALSE, message=FALSE, warning=FALSE}
...
#### Bar Chart, Transformed Variable
```{r host_response_rate trans bar, echo=FALSE, message=FALSE, warning=FALSE}
bar621(inp,
...
```{r host_response_rate trans price, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r host_listings_count price, echo=FALSE, message=FALSE, warning=FALSE}
scat621(inp,log(host_listings_count), price)
```  
  
#### Transformation  
  
```{r host_listings_count trans, echo=FALSE, message=FALSE, warning=FALSE}
...
#### Bar Chart, Transformed Variable
```{r host_listings_count trans bar, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r host_listings_count trans price, echo=FALSE, message=FALSE, warning=FALSE}
...
The next predictive variable we will look at is 'room_type.' This variable is listed as character and we would like to convert to a factor. We see that Shared room have only 178 (1 percent) while the majority of the other entries are split between Entire home/apt and Private room. There are no missing entries in this variable. We also plotted the room_type by price. The output is below and it shows quite a bit of outliers.  
...
The next variable that we will look at is 'accommodates'. This variable tells us how many people each listing is suitable for. It is an integer value and we believe we can keep it as such. The plot for missing data above showed it had no missing values. We'll go ahead and obtain the summary stats. 
...
bar621(inp, factor(accommodates),
       xlab=tmp,
       title=paste("Frequency of levels of",tmp)) + ylim(c(0,10050))
...
The minimum value for this variable is 1 and the max is 16. We have a mean of 3.324. We have a skewness of 1.77, which is not terrible. There are no NA's and about 210 entries are outliers. There does seem to be some sort of relationship so I think this variable is worth keeping. 
...
bar621(inp, factor(security_deposit),
       xlab=tmp,
       title=paste("Frequency of levels of",tmp)) + ylim(c(0,10050))
...
bar621(inp, factor(log(security_deposit)),
       xlab=tmp,
       title=paste("Frequency of levels of",tmp)) + ylim(c(0,10050))
...
impute.data <- inp[,c("bathrooms", "bedrooms", "beds", "security_deposit", "cleaning_fee")]

impute.data <- mice(impute.data, m = 5, maxit = 50, meth = 'pmm', seed = 500)
...
bar621(inp, factor(log(bathrooms)),
       xlab=tmp,
       title=paste("Frequency of levels of",tmp)) + ylim(c(0,10050))
...
Our next few variables are characters and we will be converting them to factors. The first variable is bed_type. As stated before, it's a factor containing 5 levels. 
...
Looking at the summary output, the majority of the variables are Real Bed (18,322). The rest of the levels are negligible. We will not be moving forward with this variable, therefore we will remove the column.
...
The next variable  that we are going to look at are guests_included. The guests included is a numerical variable that tells you how many additional guest are allowed. There's a fee associated with this variable so I see the price being affected by this variable. 

```{r summary guest, echo=FALSE, message=FALSE, warning=FALSE}
kable(stats(data.frame(inp$guests_included)))

tmp = "guests_included"

bar621(inp, factor(guests_included),
       xlab=tmp,
       title=paste("Frequency of levels of",tmp)) + ylim(c(0,10050))
...
#check if log looks better
#
kable(stats(data.frame(log(inp$guests_included))))

tmp = "guest_included"

bar621(inp, factor(log(guests_included)),
       xlab=tmp,
       title=paste("Frequency of levels of",tmp)) + ylim(c(0,10050))

box621(inp, log(guests_included), price)

scat621(inp, log(guests_included),  price)
#log does not improve stats so we will not replace in our inp dataset
```
...
```{r review_scores_cleanliness, message=FALSE, warning=FALSE}
...
GG.inp$review_scores_accuracy = recode(inp$review_scores_accuracy,"10='High'; 8:9='Mid'; 2:7='Low'")
GG.inp$review_scores_cleanliness = recode(inp$review_scores_cleanliness,"10='High'; 8:9='Mid'; 2:7='Low'")
GG.inp$review_scores_checkin = recode(inp$review_scores_checkin,"10='High'; 8:9='Mid'; 2:7='Low'")
```
...
bar621(GG.inp, review_scores_accuracy,
       title="review_scores_accuracy by type",xlab="review_scores_accuracy")

box621(GG.inp,review_scores_accuracy, log(GG.inp$price),
...
bar621(GG.inp, review_scores_cleanliness,
       title="review_scores_cleanliness by type",xlab="review_scores_cleanliness")

box621(GG.inp, review_scores_cleanliness, log(GG.inp$price),
...
### review_scores_checkin
```{r review_scores_checkin, echo=FALSE, message=FALSE, warning=FALSE}
bar621(GG.inp, review_scores_checkin,
       title="review_scores_checkin by type", xlab="review_scores_checkin")

box621(GG.inp, review_scores_checkin, log(GG.inp$price),
...
```{r review_scores_communication, echo=FALSE, message=FALSE, warning=FALSE}
kable(stats(data.frame(inp$review_scores_communication)))
```

#### Relative frequency

```{r review_scores_communication freq, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r reveiew_scores_communication trans, echo=FALSE, message=FALSE, warning=FALSE}
inp = as.tbl(inp)
...
```{r review_scores_communication price, echo=FALSE, message=FALSE, warning=FALSE}
box621(inp, review_scores_communication_bin, price)
```  
...
```{r review_scores_location dist, echo=FALSE, message=FALSE, warning=FALSE}
kable(stats(data.frame(inp$review_scores_location)))
```

#### Relative frequency

```{r review_scores_location, echo=FALSE, message=FALSE, warning=FALSE}
table(inp$review_scores_communication, useNA = "always")
```


```{r review_scores_location bar}
...
```{r review_scores_location trans, echo=FALSE, message=FALSE, warning=FALSE}
inp = as.tbl(inp)
...
```{r review_scores_location price, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r review_scores_value, echo=FALSE, message=FALSE, warning=FALSE}
kable(stats(data.frame(inp$review_scores_value)))
```

#### Relative frequency

```{r review_scores_value freq, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r review_scores_value trans, echo=FALSE, message=FALSE, warning=FALSE}
inp = as.tbl(inp)
...
```{r review_scores_value price, message=FALSE, warning=FALSE}
box621(inp, review_scores_value_bin,price)
``` 
...
```{r}
box621(inp, cancellation_policy_bin, price)
``` 
...
```{r}
scat621(inp %>% sample_n(5000), calculated_host_listings_count^(-0.6), price)
...
```{r}
scat621(inp %>% sample_n(5000), reviews_per_month^(0.2), price)
