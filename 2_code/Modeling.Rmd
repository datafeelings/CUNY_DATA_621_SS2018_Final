...
ggplot(aes(x=fit, y = actual),data=step_pred_ci) +
  ...
  geom_path(aes(x=fit,y=upr,color=I("red"))) +
...
As each of the discussed predictors is a factor variable, each individual level can be understood as presence/absence of a particular parameter in a given listing (e.g. precence of the AC amenity). As the response was log-transformed in the model, the effect of the predictors is multiplicative, meaning that the coefficient of the predictor indicates a percentage increase in the value of the response variable. The effects of the major predictors are summarized accordingly in the table below.

```{r}
kable(rank_var_imp(step_model)[1:20,])
```
